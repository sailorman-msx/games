...
;--------------------------------------------
; SUB-ROUTINE: MathAbsolute
; Aレジスタの値を絶対値に変換して返却する
;--------------------------------------------
MathAbsolute:

    bit 7, a ; Aレジスタの第7ビットを反転し、Zフラグにセットする
    ret z    ; Zフラグが成立したら（＝第7ビットが0だったら)そのまま返却
    neg      ; 2の補数を取得してAレジスタに格納しなおす
    ret
...
;--------------------------------------------
; SUB-ROUTINE: CalcDividBy2
; 8bitの割り算を行う
; Dレジスタに格納されている値を2で割り
; 商をHレジスタに余をLレジスタに格納して返却する
;
; D / 2 -> H
; D % 2 -> L
;--------------------------------------------
CalcDivideBy2:
    ...
    ; 掛け算の結果をBCレジスタから減算して余りを出す
    sbc hl, bc ; Lレジスタには余りが格納される
    ld h, d    ; 商をHレジスタにセットする
