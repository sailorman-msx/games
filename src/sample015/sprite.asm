...
CheckVRAM4x4Up:
    ...
    cp '&' ; 移動先はブロックか？
    jp z, CheckVRAM4x4DoNotMove
    cp '#' ; 移動先はブロックか？
    jp z, CheckVRAM4x4DoNotMove
    cp $72 ; 移動先はドアか？
    jp z, CheckVRAM4x4DoNotMoveDoor
...
;------------------------------------------------
; SUB-ROUTINE: GetCollisionItem
; プレイヤーのタイル上に重なっているアイテムを調べる
; 同時に発射できる弾は画面上で2発までとする
;------------------------------------------------
GetCollisionItem:
    ...
    ; プレイヤーのMAP座標に重なっているタイル番号を調べる
    ld a, (WK_PLAYER_MAPPOSY)
    or 0
    jp z, GetCollisionItemLoopEnd

    ld b, a
    ld de, 45

GetCollisionItemLoop:

    add hl, 45
    djnz GetCollisionItemLoop

GetCollisionItemLoopEnd:
...
;--------------------------------------------
; SUB-ROUTINE: GetPlayerMapPos
; プレイヤーの座標をMAP座標に変換する
;--------------------------------------------
GetPlayerMapPos:

    ; プレイヤーのX座標をMAP座標に変換する
    ld a, (WK_PLAYERPOSX)
    srl a ; X座標を2で割りタイル座標にする
    ld d, a ; D = A / 2

    ld a, (WK_VIEWPORTPOSX)
    add a, d  ; A = A + D

    ld (WK_PLAYER_MAPPOSX), a ; プレイヤーのMAP座標Xをセット

    ; プレイヤーのY座標をMAP座標に変換する
    ld a, (WK_PLAYERPOSY)
    srl a ; Y座標を2で割りタイル座標にする
    ld d, a ; D = A / 2

    ld a, (WK_VIEWPORTPOSY)
    add a, d  ; A = A + D

    ld (WK_PLAYER_MAPPOSY), a ; プレイヤーのMAP座標Yをセット
...
;--------------------------------------------
; SUB-ROUTINE: MovePlayerTileReset
; プレイヤーのMAP座標のタイル番号を床(0)にする
;--------------------------------------------
MovePlayerTileReset:
...
MovePlayerTileResetLoop:

    add hl, 45
    djnz MovePlayerTileResetLoop

MovePlayerTileResetLoopEnd:
