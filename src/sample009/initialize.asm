...
WK_PLAYERDISTOLD:equ $D008  ; 1バイト（プレイヤーの向き:1=上,5=下,7=左,3=右)
...
WK_PLAYERSPRATTR:equ $D011  ; 8バイト（スプライト2枚分）
WK_PLAYERSPRCLR1:equ $D019  ; 1バイト（スプライト1枚目)
WK_PLAYERSPRCLR2:equ $D01A  ; 1バイト（スプライト2枚目)

; VRAM情報取得用ワークテーブル
; 指定されたXY座標周辺のVRAM情報を取得する
WK_VRAM4X4_TBL:equ $D01B    ; 16バイト（VRAM情報格納用）
WK_CHECKPOSX:equ $D02B      ; 1バイト（VRAM情報取得用X座標）
WK_CHECKPOSY:equ $D02C      ; 1バイト（VRAM情報取得用Y座標）
