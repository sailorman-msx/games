...
CreateSpritePattern:
    ...
    ld bc, 1984    ; パターン番号#0-#247までの248個x8バイト
    call WRTVRMSERIAL
...
;--------------------------------------------
; SUB-ROUTINE: ChangeSpritePatternWater
; スプライトパターンの下半分を更新する
;--------------------------------------------
ChangeSpritePatternWater:
...
ChangeSpritePatternWaterLoop:

    ld bc, (WK_BCREGBACK)

    add hl, bc ; 8バイトポインタを進める
    ld bc, 8   ; HLレジスタのアドレスの内容を8バイトぶん0x00にする
    ld  a, $00
    call WRTVRMFIL
    
    ld a, (WK_VALUE01)
    dec a
    jp z, ChangeSpritePatternWaterEnd

    ld (WK_VALUE01), a
    jp ChangeSpritePatternWaterLoop

ChangeSpritePatternWaterEnd:

    ret

;--------------------------------------------
; SUB-ROUTINE: ChangeSpritePatternDefault
; スプライトパターンの下半分を元に戻す
;--------------------------------------------
ChangeSpritePatternDefault:

    ; スプライトパターンの下半分を構築しなおす
    ld hl, SPRPTN
    ld de, $3800
    ld bc, 832
    call WRTVRMSERIAL
...
SetVirtAttrTable:
    ...
    ; 攻撃アイテムを保持してない場合は
    ; スプライト1枚目のY座標を強制的に0にする
    ld a, (WK_EQUIP_STR)
    or a
    jr nz, SetVirtAttrTableShuffleProc
