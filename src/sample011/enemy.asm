...
GetEnemyRange:
    ...
    and 00001111B              ; 00001111B でANDして0-15までの値にする

    inc a                      ; 移動距離は1-16(単位はタイル)
...
;
; (仕様)
; テキの論理座標とビューポート左上のMAP論理座標との距離（差）が12以上の場合
; テキの移動は行わない。
; 
...
;
; テキの進行カウンタが8以上の場合は進行方向に半タイルぶん移動させる。
; この場合は論理座標は移動させない。
; *この処理はmap.asmで実装（現在、未実装）
;
; テキの論理座標が12x12ビューポート範囲外の場合:
; 移動方向先の1タイルが床や炎、穴ではない場合は移動せずに、移動情報を初期化する。
; 
; テキの論理座標が12x12ビューポート範囲内の場合:
; 半タイルの移動先が床や炎、穴ではない場合は移動させずに、移動情報を初期化する。
...
MoveEnemiesLoop1:
    ...
    ;-----------------
    ; テキの論理座標とビューポート左上のMAP論理座標が12以上の場合、テキの移動は行わない。
    ;-----------------
    ...
    cp b                ; IF ENEMYPOSX - WK_VIEWPORTPOSX

    jp c, MoveEnemiesLoop1End   ; テキキャラとの横距離が範囲外
    ...
    cp b                ; IF ENEMYPOSY - WK_VIEWPORTPOSY
    jp c, MoveEnemiesLoop1End   ; テキキャラとの縦距離が範囲外
